: $Id: vecst.mod,v 1.413 2007/12/24 14:26:36 billl Exp $
...
static double fewind (void* vv) {
  ...
  if (!flag && nx!=ni) printf("fewind WARNING nx!=ni: %d!=%d\n",nx,ni);
  for (i=0;i<num;i++) { 
  ...
  if (flag) for (i=0;i<nx;i++) if (ix[i]==0) {printf("fewind ERR2 %d\n",i); hxe();}
  for (j=0;j<num;j++) {
    for (i=0;i<nx;i++) x[i]=vvo[j][scr[i]];    
    for (i=0;i<nx;i++) vvo[j][i]=x[i];   
    list_vector_resize(ob, j, nx);
  }
  if (flag) free(ix);
  return (double)ni;
}
...
: v1.uniq(List,1) L.o(0) has the uniq values with preserved order
VERBATIM
...
PROCEDURE install_vecst () {
  if (VECST_INSTALLED==1) {
    printf("$Id: vecst.mod,v 1.413 2007/12/24 14:26:36 billl Exp $\n")
  } else {
  VECST_INSTALLED=1
  VERBATIM {
  int i,j; 
